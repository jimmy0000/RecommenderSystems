% Version 1.000
%
% Code provided by Ruslan Salakhutdinov - modified by Arvind Sundararajan
% 
...
for iteration = 0:49
    fold = mod(iteration,10);
    domain = int32(fix(iteration / 10));
    %disp(domain);
    domainstr = 'action';
    switch domain
        case 0
    ...
    trainfile = strcat('/nethome/asundararajan3/cse6240hw2/pmfcmfdata/',domainstr, 'train', int2str(fold), '.txt');
    %trainfile = strcat('/nethome/asundararajan3/cse6240hw2/pmfcmfdata/combinedtrain.txt');
    testfile = strcat('/nethome/asundararajan3/cse6240hw2/pmfcmfdata/',domainstr, 'test', int2str(fold), '.txt');
    disp(testfile);
    rand('state',0); 
    randn('state',0); 
    ...
    train_vec = importdata(trainfile);
    probe_vec = importdata(testfile);
    ...
    for epoch = epoch:maxepoch
      ...
      for batch = 1:numbatches
        ...
        aa_p   = double(train_vec(:,1));
        aa_m   = double(train_vec(:,2));
        rating = double(train_vec(:,3));
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      if err_valid(epoch) < bestRMSE
          bestRMSE = err_valid(epoch);
      end
